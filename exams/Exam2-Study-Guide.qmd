...
> Models could also be compared as **Simpler** or **More Complex** based on their relative number of parameters
...
* Sampling distribution under the null hypothesis is called **Null Distribution**:
 - Null Distribution is used to determine evidence against the null $(H_0)$:
    - **Rejection Region** determines the far values based on a significance level $\alpha$ and the alternative $H_a$.
    - The $p$-value measures how unexpected the observed sample statistic is, in favor of the alternative $H_a$. 
...
4. Compare **$p$-value** to $\alpha$ or observed test statistic to **rejection region**.
5. Make a decision and write a conclusion
6. check assumptions.

### Interpretation

* Small $p$-value or observed test statistic in the rejection region $\to$ evidence against $H_0$.
* Large p-value or observed test statistic outside the rejection region $\to$ not enough evidence against $H_0$.
...
* Observation: $n$ observations: $y_1, y_2, ..., y_n$
...
$$y=\beta_0 + \beta_1X+\varepsilon, \text{ where } X=\left\{\begin{align}0 & \text{ for group A}\\1 & \text{ for group B}\end{align} \right.$$

* Predicted values: 
  $$\hat y_i = \left\{
    \begin{align} \bar y_A & \text{ for group } A\\
...
* Statistics:
  ...
  $$SE = s_{\bar y_A - \bar y_B} = \left\{\begin{align}\sqrt{\frac{s_A^2}{n_A} + \frac{s_B^2}{n_B}} & \text{ general case }\\ \sqrt{\frac{s_p^2}{n_A} + \frac{s_p^2}{n_B}} & \text{ assuming } \sigma_A=\sigma_B \end{align}\right.$$
