...
execute:
  ...
  warning: false
  message: false
output-files: CK-ch6_III-sec5-11.html
---
...
## Application of $Z$: <br/> Within Group Comparison

:::{.columns}
:::{.column}
```{r}
favstats(Height~Gender, data=Fingers)[,c(1,7:8)]
```
:::
:::{.column}
...
cat('\n')
cat(sprintf('Female: Z = (%0.2f-%0.2f)/(%0.3f) \n\t= %0.3f', 
  heights[1], HGmeans['female'], HGsd['female'], 
  (heights[1]- HGmeans['female'])/HGsd['female']), '\n',
  '\n',
  sprintf('Male: Z = (%0.2f-%0.2f)/(%0.3f) \n\t= %0.3f', 
...
## Application of $Z$: <br/> Between Group Decision
...
```{r, result='asis'}
cat(sprintf('Female: P(Height>68)=P(Z>%0.2f)=%0.4f', 
  (68-HGmeans[1])/HGsd[1],
  mean(Fingers$Height[Fingers$Gender=='female']>68)), '\n',
  sprintf('Male: P(Height<68)=P(Z<%0.2f)=%0.4f', 
  (68-HGmeans[2])/HGsd[2],
  mean(Fingers$Height[Fingers$Gender=='male']<68)), '\n', sep='')
```
:::
